...
echo "Please run below command to run the docker locally:" > ${target_ouput_dir}/docker_tag.log
echo "sudo docker run --net=host -d -t rackhdci/${docker_tag}" >> ${target_ouput_dir}/docker_tag.log
echo "PS: the docker require a NIC with ip 172.31.128.250" >> ${target_ouput_dir}/docker_tag.log
